...
from .models import MonitorTypes, Resume
...
@receiver(signal=post_save, sender=MonitorTypes, dispatch_uid="job_post_save_dispatcher")
def post_save_callback(sender, instance=None, created=False, **kwarg):
    message = ""
    if isinstance(instance, MonitorTypes):
        message = "Monitor type for %s has been saved" % instance.monitor_type
    else:
